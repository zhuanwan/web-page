...
function getTotalLinePoints(leafInstance, instances) {
  ...
  while (lineInstance) {
    ...
    linePoints.push({ x: x2 + 1, y: y2 + 1 }, { x: x1 + 1, y: y1 + 1 })
    lineIds.push(nodeId)
...
function animate() {
  ...
  for (let m = 0; m < keys.length; m++) {
    ...
    polylineInstance.set({
      strokeDashOffset: num,
    })
    // 设置动画的z-index比开关小一点，以免遮住开关
    canvas.moveTo(polylineInstance, zIndex)

    idToPathAndRect[k].offset = num % 100
  }
...
function init(canvas, data) {
  ...
  // 先把所有的开关的z-index提到最高
  const switchInstances = instances.filter(ele => ele.isSwitch)
  switchInstances.forEach(s => {
    canvas.moveTo(s, instances.length);
  });
  const switchInstancesLen = switchInstances.length
  ...
  canvas.on('mouse:down', function(options) {
    ...
    if (options.target.isLeaf) {
      ...
      if (!idToPathAndRect[id]) {
        ...
        const polylineInstance = new fabric.Polyline(linePoints, {
          fill: 'transparent',
          // fill: '#00f',
          stroke: 'red',
          strokeWidth: 2,
        ...
        canvas.add(polylineInstance)

        idToPathAndRect[id] = {
          polylineInstance,
          offset: 0, // 偏移
          zIndex: instances.length - switchInstancesLen,
          pause: true, // 是否暂停
          lineIds,
        }
      }
      ...
      Object.keys(idToPathAndRect).forEach((k) => {
        ...
        if (isLineSwitchOpen(lineIds)) {
          canvas.remove(idToPathAndRect[k].polylineInstance)
...
var stats = initStats()
let idToPathAndRect = {} // 正在动画的路径
let idToSwitchMap = {} // 所有开关的状态
